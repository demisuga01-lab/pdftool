...
const sections: Array<ControlSection<ProtectSettings>> = [
  {
    ...
    render: (settings, update) => (
      <div className="space-y-3">
        ...
        <div className="space-y-1.5">
          <label className="block text-[13px] text-slate-700 dark:text-zinc-300">Confirm password</label>
          <input
            className="h-9 w-full rounded-md border border-slate-200 bg-white px-3 text-[14px] text-slate-700 outline-none focus:ring-2 focus:ring-[#10B981] focus:ring-offset-1 dark:border-white/10 dark:bg-zinc-950 dark:text-zinc-100 dark:focus:ring-emerald-400 dark:focus:ring-offset-zinc-900"
        ...
        <div className="flex items-center justify-between gap-3 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 text-[13px] text-slate-600 dark:border-white/10 dark:bg-zinc-950 dark:text-zinc-300">
          <span>
            Password strength:{" "}
            {settings.userPassword.length >= 12 ? "Strong" : settings.userPassword.length >= 8 ? "Medium" : "Weak"}
          </span>
          <button
            className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-slate-700 transition hover:bg-slate-50 dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:bg-white/5"
            onClick={() => {
