...
export function UploadProgress({
  ...
  return (
    <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-900/10 dark:border-white/10 dark:bg-zinc-900 dark:shadow-black/35">
      <div className="space-y-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            <p className="text-sm font-semibold text-[#059669] dark:text-emerald-300">{fileLabel ?? "Uploading file"}</p>
            <h2 className="text-4xl font-bold text-slate-900 dark:text-zinc-100">{percent}%</h2>
          </div>
          {onCancel ? (
