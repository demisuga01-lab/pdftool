      ...
      <div className="group overflow-hidden rounded-xl border border-zinc-200 bg-white transition hover:shadow-sm dark:border-white/10 dark:bg-zinc-900">
        <div className="relative">
          <button
            ...
            className="absolute right-3 top-3 z-20 inline-flex h-10 w-10 items-center justify-center rounded-lg border border-zinc-200 bg-white text-slate-400 transition hover:text-slate-700 dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-500 dark:hover:text-white"
            type="button"
            {...attributes}
            {...listeners}
          >
          ...
          <div className="aspect-[4/3] bg-zinc-50 p-3 dark:bg-zinc-950">
            <img alt={title} className="h-full w-full rounded-lg border border-zinc-200 object-contain dark:border-white/10" src={preview} />
          </div>
        </div>
        <div className="space-y-1 border-t border-zinc-200 px-3 py-2 dark:border-white/10">
          ...
          {items.map((item) => (
            <SortableImageCard
              hoverActions={
                onRemove ? (
                  <button
                    className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-slate-700 dark:border-white/15 dark:bg-zinc-900 dark:text-zinc-200"
  ...
  return (
    <main className="flex min-h-[calc(100dvh-3.5rem)] flex-col overflow-x-hidden bg-white dark:bg-zinc-950 sm:min-h-[calc(100dvh-4rem)] lg:h-[calc(100dvh-4rem)]">
      ...
      <div className="relative flex min-h-0 flex-1 flex-col lg:flex-row">
          ...
          {hasContent ? (
            <>
              {(showSizeToggle && setSize) ? (
                <div className="flex items-center justify-between gap-3 border-b border-zinc-200 bg-white px-4 py-2.5 dark:border-white/10 dark:bg-zinc-950 sm:px-6">
                      ...
                      return (
                        <button
                          ...
                          className={[
                            ...
                            size === option.key
                              ? "border-[#059669] bg-[#ECFDF5] text-[#059669] dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-300"
                ...
                <div className="space-y-5">
                  ...
                  <section className="rounded-2xl border border-zinc-200 bg-white p-4 pb-6 dark:border-white/10 dark:bg-zinc-900 lg:hidden">
                    <div className="mb-4 space-y-1">
                      <h2 className="text-[18px] font-bold text-slate-900 dark:text-zinc-100">{breadcrumbTitle}</h2>
                    ...
                    <div className="mt-6">
                      ...
                      <p className="mt-2 text-center text-xs text-slate-400 dark:text-zinc-500">
                        {estimatedTime ?? PUBLIC_FILE_HANDLING_STATEMENT}
                      </p>
                    </div>
                  </section>
                </div>
        ...
        {hasContent ? (
          <div className="fixed inset-x-0 bottom-0 z-30 border-t border-zinc-200 bg-white p-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] dark:border-white/10 dark:bg-zinc-950 lg:hidden">
  ...
  return (
    <label
      className={[
        ...
        dragging ? "border-[#059669] bg-[#ECFDF5] dark:bg-emerald-950" : "border-slate-300 dark:border-white/15",
      ].join(" ")}
